  ...
  return (
    <div className="min-h-screen" dir={dir} lang={lang}>
      <header className="sticky top-0 z-40 border-b border-border/60 backdrop-blur-md bg-background/70">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          <Link to="/" className="flex items-center gap-2 font-display font-semibold">
            ...
            <span><span className="block leading-none">Amr Elbusaily</span><span className="mt-1 block font-mono text-[9px] font-normal uppercase tracking-[0.16em] text-muted-foreground">SEO & AI Growth</span></span>
          </Link>
      ...
      <article className="mx-auto max-w-3xl px-6 py-14 md:py-20">
        ...
        <h1 className="mt-4 text-4xl font-semibold leading-[1.08] md:text-5xl">{title}</h1>
        <p className="mt-5 border-s-2 border-primary/60 ps-5 text-lg leading-8 text-muted-foreground">{description}</p>
        <div className="article-prose mt-12 space-y-6 text-base leading-8 text-foreground/90 [&_h2]:mt-10 [&_h2]:text-2xl [&_h2]:font-semibold [&_h2]:text-foreground [&_h3]:mt-6 [&_h3]:text-xl [&_h3]:font-semibold [&_ul]:list-disc [&_ul]:space-y-2 [&_ol]:list-decimal [&_ol]:space-y-2 [&_ul]:ps-6 [&_ol]:ps-6 [&_a]:text-primary [&_a]:underline [&_strong]:text-foreground">
          {children}
        </div>
        ...
        <div className="mt-16 rounded-2xl border border-border bg-surface p-8">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <a
              href={lang === "ar" ? "/ar/free-consultation" : "/free-consultation"}
              className="inline-block rounded-md bg-primary px-5 py-3 text-sm font-medium text-primary-foreground"
            >
              {lang === "ar" ? "خد مراجعة SEO مجانية" : "Get a free SEO review"}
            </a>
            <Link
              to={servicesHref}
